...
// ── Line logo paths — sequential left to right, circle last
const LINE_PATHS = [
  // left triangle (x: 0~230)
  "M141.327 5.46094C144.76 -0.487135 153.335 -0.487001 156.768 5.46094L230.355 132.99C217.849 123.938 199.033 126.79 190.512 141.558L131.429 243.949C125.717 253.848 115.156 259.947 103.726 259.947H9.93066C3.0679 259.947 -1.2244 252.506 2.21094 246.553L141.327 5.46094Z",
  // center triangle (x: 170~405)
  "M312.428 5.50488C315.86 -0.443598 324.437 -0.443609 327.869 5.50488L401.455 133.032C388.948 123.981 370.134 126.834 361.612 141.602L302.529 243.992C296.817 253.891 286.256 259.99 274.826 259.99H181.031C174.169 259.99 169.876 252.549 173.312 246.596L312.428 5.50488Z",
  // right triangle (x: 370~461)
  "M409.33 169.828C412.126 165.036 419.052 165.036 421.848 169.828L460.865 236.706C466.889 247.032 459.44 260 447.483 260H383.695C371.739 260 364.29 247.032 370.313 236.706L409.33 169.828Z",
  // circle — drawn last
  "M234.594 1.04297C252.03 1.04309 266.165 15.1767 266.165 32.6104C266.165 50.0439 252.03 64.1766 234.594 64.1768C217.158 64.1768 203.023 50.044 203.022 32.6104C203.022 15.1766 217.157 1.04297 234.594 1.04297Z",
]
const LINE_DELAYS = [0, 0.45, 0.85, 1.3]
const LINE_DURATIONS = [1.2, 1.1, 0.6, 0.7]

// ── Fill logo paths — circle, left, center, right
const FILL_PATHS = [
  // circle (top center)
...
function PathDrawDemo() {
  ...
  return (
    <div style={styles.card}>
      ...
      <p style={styles.desc}>Each shape draws in reverse, left to right. Circle appears last.</p>
...
// ── 2. Stagger Reveal (fill logo)
function StaggerDemo() {
  const [key, setKey] = useState(0)
  return (
    <div style={styles.card}>
      <p style={styles.label}>② Stagger Reveal</p>
      <p style={styles.desc}>Shapes fade in sequentially — circle, left, center, right.</p>
      <div style={styles.svgWrap}>
        <motion.svg
          key={key}
          viewBox="0 0 464 261"
...
function ScaleFadeDemo() {
  ...
  return (
    <div style={styles.card}>
      ...
      <p style={styles.desc}>Logo scales up from 82% while fading in.</p>
      <div style={styles.svgWrap}>
        <motion.svg
          key={key}
          viewBox="0 0 464 261"
